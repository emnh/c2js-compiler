...
function Interpreter() {
  ...
  I.m = function() {
    ...
    a.execute = function*(state) {
      ...
      lvalue.get = function(state) {
        ...
        var value = record.getField(state, member);
        if (value == null) {
          logger.log(state, "record.getField", member, record, record.getField(member));
          throw new Error("null member lookup: " + member);
        }
        //console.log("record.getField: ", member, record.getField(member));
        return value;
      };
      lvalue.set = function(state, val) {
        ...
        if (record.getField(state, member) == null) {
          throw new Error("null member lookup: " + member);
        }
  ...
  I.fill_string = function() {
    var a = new ASTNode("fill_string", arguments);
    a.execute = function*(state) {
      var [state, value] = yield [state, a.children[0]];
      var [state, size] = yield [state, a.children[1]];
      var result = new StringPointer(Array.apply(null, new Array(size)).map(function() { return String.fromCharCode(value); }, 0).join(''));
      yield [state, result];
    }
    return a;
  }
  ...
  I.subscript = function() {
    ...
    a.execute = function*(state) {
      ...
      if (!(arLValue instanceof IndexedPointer)) {
        throw new Error("tried to subscript !IndexedPointer");
      }
      var lvalue = new LValue();
      lvalue.get = function(state) {
...
function DOMLogger() {
  ...
  D.log = function(state) {
    tds = [H.td("log")];
    for (var i = 1; i < arguments.length; i++) {
      tds.push(H.td(D.formatDefault(state, arguments[i])));
    }
