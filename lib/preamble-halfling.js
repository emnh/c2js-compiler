...
function DOMLogger() {
  ...
  D.getClickFun = function(leftContainer, frame) {
    return function(){
      //console.log("state", mori.toJs(frame.state.data));
      var frameElement = $("#frame");
      frameElement.html(D.formatFrame(frame));
      frameElement.css({ 
...
function runEvalNodeInternal(I, stack, callback) {
      ...
      switch (frame.stage) {
        ...
        case 3:
          if (!frame.item.done) {
            ...
            if (frame.state.data === undefined) {
              throw new Error("incorrect state yield");
            }
            if (childNode instanceof I.ASTNode) {
              ...
              console.log("invalid childNode, faulting generator to show stack");
              while (true) {
  ...
  return evalNodeInternal().next().value;
}
...
function runMain() {
  ...
  var [state, result] = evalNode(I, astStack, astPrograms, state, callback);
  console.log("result", result);
}
